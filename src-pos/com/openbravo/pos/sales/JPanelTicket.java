...
//    private ScriptObject scriptobjinst;
    ...
    private void addTicketLine(ProductInfoExt oProduct, double dMul, double dPrice) {   
        ...
        double sellPrice = dPrice;

        if ( oProduct.getPriceSell2() != 0.0 ) {
            // pricesell1, pricesell2
            Object[] options = {"MARKET", "KLIENT" };
                int n = JOptionPane.showOptionDialog(null,
                    "Cilin cmim te perdor per kete produkt ?",
                    "Cmimi i produktit",
             ...
             if( n == 0 ) {
                 sellPrice = oProduct.getPriceSell();
                 dPrice = oProduct.getPriceSell2();
             } else {
                 sellPrice = 0.0;
             }
        }
        ...
        addTicketLine(new TicketLineInfo(oProduct, dMul, dPrice, sellPrice, tax, (java.util.Properties) (oProduct.getProperties().clone())));
    }
    ...
    private void incProduct(ProductInfoExt prod) {
        ...
        if (prod.isScale() && m_App.getDeviceScale().existsScale()) {
            try {
                Double value = m_App.getDeviceScale().readWeight(prod.getName());
                if (value != null) {
